...
vanDaalen_NLBE <- 
        read.csv(here::here("data-raw", "vanDaalen_NLBE_unpublished.csv"), sep = ",") |> 
        dplyr::select(n_sapwood) |>
        dplyr::count(n_sapwood) |>
        dplyr::rename("count" = "n") |> 
        dplyr::filter(!is.na(count))

attr(vanDaalen_NLBE, "citation") <-
        "Sjoerd van Daalen (Van Daalen Dendrochronologie, the Netherlands), unpublished data"
attr(vanDaalen_NLBE, "area") <- "Historical timbers found in the Netherlands en Belgium, with a local provenance."

usethis::use_data(vanDaalen_NLBE, overwrite = TRUE)
