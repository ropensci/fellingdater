...
sw_model_plot <-
     function(x,
          ...
          max <- x$range[3]
          max_count <- max(x$sapwood_model$count, na.rm = TRUE)
          ...
          plot <- ggplot2::ggplot() +
               ggplot2::geom_col(
                    ...
                    data = dplyr::filter(x$sapwood_model, !is.na(count) & !is.na(n_sapwood)),
                    ggplot2::aes(
                         x = n_sapwood,
                         y = count
                    ),
               ...
               ggplot2::scale_y_continuous(breaks = seq(0, max_count, by = if (max_count > 40) 10 else 5)) +
               ggplot2::labs(
                    subtitle = paste0(
                         ...
                         round(x$hdi_model$p * 100, 1), "%)</span>: between ",
                         hdi_lower,
                         " and ",
                         hdi_upper,
                         " sapwood rings"
               ...
               theme_fdr() +
               ggplot2::theme(
                    plot.subtitle = ggtext::element_markdown(hjust = 0, lineheight = 1.2),
