...
sw_combine_plot <- function(x) {
     ...
     p <- ggplot2::ggplot(pdf, ggplot2::aes(x = .data$year, y = .data$p)) +
               ...
               if (length(tpq) > 0) {
                    ggplot2::geom_point(
                         data = pdf |>
                              ...
                              dplyr::summarize(max = max(.data$year)),
          ...
          {
               if (any(!is.na(summary$lower)) && any(!is.na(summary$upper))) {
                    ggplot2::geom_segment(
                         data = summary,
                         ggplot2::aes(
                              x = .data$lower,
          ...
          # # add the combined felling date estimate as background
          {
               if (!is.na(hdi[[2]])) {
                    ggplot2::geom_area(
                         data = combo,
                         ggplot2::aes(x = .data$year, y = .data$comb),
                         fill = "grey50",
                         alpha = 0.5,
                         na.rm = TRUE
                    )
          ...
          ggplot2::geom_text(
               ...
               ggplot2::aes(
                    ...
                    label = paste0(
                         ...
                         .data$last,
                         ", swr: ",
                         ifelse(is.na(.data$n_sapwood), "- ", as.character(.data$n_sapwood)),
                         ")",
                         "\n "
                    ),
                    hjust = 1
          ...
          ggplot2::geom_text(
               ...
               ggplot2::aes(
                    x = ceiling((range[2]) / 10) * 10,
                    y = 0.40,
                    label = ifelse(is.na(.data$agr_index),
                         "",
                         paste0("A[i]", "==", round(
                              as.numeric(.data$agr_index), 1
                         ), "*\'%'")
                    ),
                    color = ifelse(.data$agr_index < 60, "poor", "good")
               ),
               hjust = 1, parse = TRUE
          ) +
