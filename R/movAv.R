...
#'
#' @description
#' This vectorized function computes a running mean/moving average
...
#' @param w The width of the moving average window. When `w`is even, one more value from the
#'   future is included.
#' @param align The alignment of the window relative to the current data point. It can be one of:
#'
...
#'  series. Options are:
#'  * `edges = "fill"`: The average is calculated with a decreasing number of values near the edges of
#'  the vector (default).
#'  * `edges = "nofill"`: The result includes `NA` values when the window does not cover w values.
#'
#' @return A `numeric` vector of the same length of `x` with the computed running mean values.
#' @export
