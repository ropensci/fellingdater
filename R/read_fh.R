...
read_fh <- function(fname,
  ...
  header.end <-
    grep(
      "^DATA:(Tree|Single|HalfChrono|Double|Quadro)$",
      inp
    )
  n <- length(header.end)
  if (n == 0) {
  ...
  ## extra header fields compared to dplR::read.fh()
  ...
  for (i in seq_len(n)) {
    ...
    ## get pith offset uncertainty (delta missing rings before start of series)
    this.missing.delta <-
      sub(
    ...
    ## get status (Dated/Undated/relDated)
    this.status <- sub(
      "DATED=",
      "",
      ignore.case = TRUE,
    ...
    ## get number of recorded SapWoodRings
    this.swr <- sub(
      "SAPWOODRINGS=",
      "",
      ignore.case = TRUE,
    ...
    ## get presence of bark from header
    this.bark <- sub(
      "BARK=",
      "",
      ignore.case = TRUE,
    ...
    ## get unmeasured, but observed rings (MissingRingsAfter)
    this.missing <-
      sub(
        "MISSINGRINGSAFTER=",
        "",
    ...
    ## get unreliable measurements - e.g. deformed rings (InvalidRingsAfter)
  ...
  ## add header fields as attributes
