...
read_fh <- function(
     ...
     header.end <- grep("^DATA:(Tree|Single|HalfChrono|Double|Quadro)$", inp)
     n <- length(header.end)
     if(n == 0) {
     ...
     for (i in seq_len(n)) {
          ...
          data_type[i]<- sub("DATA:", "", ignore.case = TRUE,
                             x = grep("^DATA:", data_type, value = TRUE,
                                      ignore.case = TRUE))

          this.header <- inp[(header.begin[i]+1):(header.end[i]-1)]
          ## get keycode (= series id)
          this.keycode <- sub("KEYCODE=", "", ignore.case = TRUE,
          ...
          # get length
          this.length <- sub("LENGTH=", "", ignore.case=TRUE,
          ...
          ## get end year
          this.end.year <- sub("DATEEND=", "", ignore.case = TRUE,
          ...
          ## get start year
          this.start.year <- sub("DATEBEGIN=", "", ignore.case=TRUE,
          ...
          this.unit <- sub("UNIT=", "", ignore.case = TRUE, # new
          ...
          ## get site code
          this.site <- sub("SITECODE=", "", ignore.case = TRUE,
          ...
          ## get tree number
          this.tree <- sub("TreeNo=", "", ignore.case = TRUE,
          ...
          ## get core number
          this.core <- sub("CoreNo=", "", ignore.case = TRUE,
          ...
          this.stemdisk <- sub("StemDiskNo=", "", ignore.case = TRUE,
          ...
          this.missing <-
               sub("MissingRingsBefore=", "", ignore.case = TRUE,
          ...
          this.status <- sub("DATED=", "", ignore.case=TRUE,
                             x = grep("^DATED=", this.header, value = TRUE,
                                      ignore.case=TRUE))
          if (length(this.status) == 1) {
               status[i] <- this.status
          }
          ## NEW: detect presence of pith
          this.pith <- sub("PITH=", "", ignore.case=TRUE,
          ...
          this.swr <- sub("SAPWOODRINGS=", "", ignore.case = TRUE,
                          x = grep("^SAPWOODRINGS=", this.header, value = TRUE,
                                   ignore.case = TRUE))
          ...
          this.waneyedge <- sub("WALDKANTE=", "", ignore.case=TRUE,
          ...
          this.bark <- sub("BARK=", "", ignore.case=TRUE,
                           x = grep("^BARK=", this.header, value = TRUE,
                                    ignore.case = TRUE))
          ...
          ## NEW: get comments
          this.comments <- sub("COMMENT=", "", ignore.case=TRUE,
          ...
          this.project <- sub("PROJECT=", "", ignore.case = TRUE,
                              x = grep("^Project=", this.header, value = TRUE,
                                       ignore.case=TRUE))
          ...
          ## NEW: get location
          this.location <- sub("LOCATION=", "", ignore.case=TRUE,
                               x = grep("^LOCATION=", this.header, value = TRUE,
                                        ignore.case = TRUE))
          ...
          ## NEW: get town name
          this.town <- sub("TOWN=", "", ignore.case=TRUE,
                           x = grep("^TOWN=", this.header, value = TRUE,
                                    ignore.case = TRUE))
          if (length(this.town) == 1) {
               town[i] <- this.town
          }
          ## NEW: get town zipcode
          this.zip <- sub("TOWNZIPCODE=", "", ignore.case=TRUE,
                          x = grep("^TOWNZIPCODE=", this.header, value = TRUE,
                                   ignore.case = TRUE))
          ...
          ## NEW: get street name
          this.street <- sub("STREET=", "", ignore.case=TRUE,
                             x = grep("^STREET=", this.header, value = TRUE,
                                      ignore.case = TRUE))
          ...
          this.author <- sub("PERSID=", "", ignore.case=TRUE,
                             x = grep("^PERSID=", this.header, value = TRUE,
                                      ignore.case = TRUE))
          ...
          ## NEW: get client id
          this.client <- sub("CLIENT=", "", ignore.case=TRUE,
                             x = grep("^CLIENT=", this.header, value = TRUE,
                                      ignore.case = TRUE))
          ...
          this.member <- sub("CHRONOMEMBERKEYCODES=", "", ignore.case=TRUE,
                             x = grep("^CHRONOMEMBERKEYCODES=", this.header,
                                      value = TRUE, ignore.case = TRUE))
          ...
          ## NEW: get latitude
          this.latitude <- sub("LATITUDE=", "", ignore.case = TRUE,
                               x = grep("^LATITUDE=", this.header, value = TRUE,
                                        ignore.case = TRUE))
          if (length(this.latitude) == 1) {
               latitude[i] <- this.latitude
          }
          # NEW: get longitude
          this.longitude <- sub("LONGITUDE=", "", ignore.case = TRUE,
                                x = grep("^LONGITUDE=", this.header, value = TRUE,
                                         ignore.case = TRUE))
     ...
     for (i in seq_len(n)) {
          ...
          } else if (data_type[i] == "Single" || data_type[i] == "Tree"){
          ...
          ## NEW: data in block format (Quadro) => chronology
               ...
               for (j in seq_along(portion)) {
                    row.fwf <- substring(portion[j],
                                         seq(from = 1, by = 6, length = 16),
                                         seq(from = 6, by = 6, length = 16)
                    )
                    row.numeric <- as.numeric(row.fwf)
          ...
          else if (data_type[i] == "Double" || data_type[i] == "HalfChrono") {
