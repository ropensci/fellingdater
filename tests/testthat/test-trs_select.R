...
test_that("trs_select validates input parameters", {
     ...
     expect_error(
          trs_select("not_a_dataframe", "trs_1"),
          "Input must be a data.frame"
     )
     expect_error(
          trs_select(list(a = 1), "trs_1"),
          "Input must be a data.frame"
     )

     # Test invalid series parameter
     expect_error(
          trs_select(x, series = 123),
          "`series` must be a non-empty character vector"
     )
     expect_error(
          trs_select(x, series = character(0)),
          "`series` must be a non-empty character vector"
     )
     expect_error(
          trs_select(x, series = c()),
          "`series` must be a non-empty character vector"
     )

     # Test invalid trim parameter
     expect_error(
          trs_select(x, "trs_1", trim = "yes"),
          "`trim` must be a single logical value"
     )
     expect_error(
          trs_select(x, "trs_1", trim = c(TRUE, FALSE)),
          "`trim` must be a single logical value"
     )
})
...
test_that("trs_select handles missing series gracefully", {
     ...
     expect_error(
          trs_select(x, series = "nonexistent"),
          "Series.*nonexistent.*not found in dataframe"
     )
